...
class AppModel {
    ...
    var currentState: String = Stats.AWAITING_START.name
    ...
    private fun setCellStatus(row: Int, column: Int, stat: Byte?)
    {
        if (stat != null)
        {
            field[row][column] = stat
        }
    }

    fun isGameOver(): Boolean
    {
        return currentState == Stats.OVER.name
    }

    fun isGameActive(): Boolean
    {
        return currentState == Stats.ACTIVE.name
    }

    fun isGameAwaitingStart(): Boolean
    {
        return currentState == Stats.AWAITING_START.name
    }

    private fun boostScore()
    {
        score += 10
        if (score >preferences?.getMaxScore() as Int)
            preferences?.saveMaxScore(score)
    }
    ...
    fun startGame()
    {
        if (!isGameActive())
        {
            currentState = Stats.ACTIVE.name
            generateNextBlock()
        }
    }
    ...
    {
        resetField(false)
        currentState = Stats.AWAITING_START.name
        score = 0
    }
}